...
async function handle({ request, env, params }) {
  ...
  const s = await sbGetOne(env, `stories?select=id,title,description,category,city,mux_playback_id,duration,vendor_name,created_at,status&id=eq.${encodeURIComponent(params.id)}&limit=1`);
  if (!s || s.status !== 'active' || !s.mux_playback_id) return render404(origin, "Cette vidéo n'est plus disponible.");

  const url = `${origin}/stories/${encodeURIComponent(s.id)}`;
  const hls = `https://stream.mux.com/${s.mux_playback_id}.m3u8`;
  const poster = `https://image.mux.com/${s.mux_playback_id}/thumbnail.jpg?width=720&fit_mode=preserve`;
  ...
  const jsonld = {
    ...
    name: title, description: desc, thumbnailUrl: [poster],
...
<video id="v" controls playsinline autoplay muted poster="${esc(poster)}"></video>
<div class="meta">
...
<script>
(function(){var v=document.getElementById('v');var src=${JSON.stringify(hls)};
