...
const DEFAULTS = {
  ...
  review_request: { subject: '⭐ Votre avis sur {{product_name}} — NEXUS Market',
    html: wrap('Comment s\'est passée votre commande ?', '<p>Bonjour {{buyer_name}},</p><p>Votre commande <strong>#{{order_id}}</strong> a bien été livrée. Comment avez-vous trouvé <strong>{{product_name}}</strong> ?</p><p>Votre avis aide les autres acheteurs à choisir et permet aux bons vendeurs de se faire connaître. Cela prend moins d\'une minute.</p>{{#if review_url}}<p style="margin-top:14px"><a href="{{review_url}}" style="background:#00853E;color:#ffffff;padding:11px 26px;border-radius:8px;text-decoration:none;font-weight:700;display:inline-block">⭐ Donner mon avis</a></p>{{/if}}{{#if product_url}}<p style="margin-top:12px;font-size:13px"><a href="{{product_url}}" style="color:#00853E">Revoir le produit →</a></p>{{/if}}') },
...
const WA_DEFAULTS = {
  ...
  review_request: '⭐ NEXUS Market — Bonjour {{buyer_name}}, votre commande #{{order_id}} a bien été livrée. Comment avez-vous trouvé {{product_name}} ? Donnez votre avis en 1 clic : {{review_url}} — ça aide les autres acheteurs et les bons vendeurs. Merci !',
