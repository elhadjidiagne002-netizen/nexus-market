...
export async function onRequest({ request, env, params }) {
  ...
  try {
    ...
    if (e) return e;
    if (!['vendor','admin'].includes(user.role)) return err('Acces vendeur requis', 403);
    ...
    const since2 = new Date(Date.now() - ms*2*86400000).toISOString();
    ...
    const prev    = await sb.from('orders').select('total',
      `vendor_id=eq.${vId}&created_at=gte.${since2}&created_at=lt.${since}&payment_status=eq.paid`) || [];
    ...
    if (route === 'vendor/chart') {
      ...
      return json({ timeline: days.map(d => grouped[d]||{date:d,revenue:0,orders:0}), totals: { revenue, orders: orders.length } });
    }
