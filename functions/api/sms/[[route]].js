...
export async function onRequest({ request, env, params }) {
  ...
  try {
    if (route === 'send-otp') {
      ...
      if ((recent?.length||0) >= 3) return err('Trop de tentatives. Reessayez dans 1h.', 429);
      ...
      const texts = { auth:`NEXUS Market: Code connexion ${code}. 10 min.`, phone_verify:`NEXUS: Verification ${code}. 10 min.`, delivery:`NEXUS: Code reception colis ${code}.` };
      const text = texts[purpose] || `NEXUS Market: Code ${code}. 10 min.`;
      const sent = await sendSms(env, num, text);
      if (!sent.ok) return err('Echec envoi SMS: ' + sent.error, 500);
    ...
    if (route === 'verify-otp') {
      ...
      if (!otps?.length) return err('Code expire. Demandez un nouveau.', 400);
      const otp = otps[0];
      if (otp.attempts >= 3) {
